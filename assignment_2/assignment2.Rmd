...
Hence, first, we estimate an Arima model on the first-differences data, using the AIC-criterion to go from general to specific models. Then, we convert back the estimates to compute the in-sample fit, and visually assess in-sample accuracy:

```{r datamining, eval = FALSE}
get_arima_model <- function(max.p, max.q){
  ...
  for(i in 1:max.p){
    ...
    for(j in 1:max.q){
      arima <- try(forecast::Arima(covid19$new_cases, order = c(i, 1, j)))
      aic_new <- try(arima$aic)
  ...
  bestmodel <- forecast::Arima(covid19$new_cases, order = c(p_best, 1, q_best))
...
```{r}
arimamodel <- forecast::Arima(covid19$new_cases, order = c(4, 1, 1),
...
We predict the values for the Arima model:
